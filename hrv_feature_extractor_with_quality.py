...
# -*- coding: utf-8 -*-
"""
HRV特征提取器 - 10秒版本（带质量评估）
从任意长度的CSV文件中提取HRV特征，并进行质量评估
输入: 包含时间和数值两列的CSV文件
输出: 一行包含6个HRV特征的数据，带质量标记

特点:
- 10秒版本，适合短时间数据分析
- 包含质量评估机制，标记高质量/非高质量数据
- 参考多线程HRV处理器的质量评估标准
- 输出格式: 6个特征值 + 质量标记

使用方法:
1. 修改代码顶部的配置参数（推荐）
2. 运行: python hrv_feature_extractor_with_quality.py --use-config
3. 或者使用命令行参数: python hrv_feature_extractor_with_quality.py input.csv -o output.csv -e "情绪标签"
"""
...
INPUT_CSV_PATH = r"C:\Users\QAQ\Desktop\emotion\010_t102743_114093_001.csv"  # 修改为你的CSV文件路径

# 输出文件路径 - 用户可在此处直接修改输出文件路径（可选）
OUTPUT_CSV_PATH = "hrv_features_with_quality.csv"  # 修改为你的输出文件路径

# 情绪标签 - 用户可在此处直接修改情绪标签
EMOTION_LABEL = "平静"  # 修改为你的情绪标签
...
# 质量评估参数 - 10秒版本，参考多线程处理器
QUALITY_PARAMS = {
    'min_peaks_per_segment': 3,     # 每段最少峰值数（10秒需要较少峰值）
    'max_peaks_per_segment': 20,    # 每段最多峰值数（10秒容纳较少峰值）
...
class HRVFeatureExtractorWithQuality:
    """带质量评估的HRV特征提取器类"""
    ...
    def extract_features_from_csv(self, csv_path: str, emotion_label: str = "unknown") -> Optional[Dict]:
        ...
        try:
            ...
            if len(data) < 50:  # 10秒版本需要较少的数据点
                print(f"❌ 数据不足: {os.path.basename(csv_path)} (需要至少50个数据点)")
                return None
            
            # 第1步：峰值检测
            peak_info, _ = self.detect_peaks(data)
            if peak_info is None:
                print(f"❌ 峰值检测失败: {os.path.basename(csv_path)}")
                return None
            
            # 第2步：生成候选数据段
            candidates = self.generate_candidate_segments(data, peak_info)
            
            # 判断是否有高质量数据段
            has_high_quality = len(candidates) > 0
            
            if has_high_quality:
                # 选择最佳数据段
                best_segment = candidates[0]
                segment_peak_times = best_segment['peak_times']
                quality_score = best_segment['quality']['quality_score']
                quality_status = "高质量"
            else:
                # 如果没有高质量段，使用所有峰值
                segment_peak_times = peak_info['times']
                quality_score = 0.0
                quality_status = "非高质量"
            
            # 从数据段中提取RR间期
            if len(segment_peak_times) > 1:
                rr_intervals = np.diff(segment_peak_times)
                
                # 计算HRV特征
                features = self.calculate_hrv_features(rr_intervals)
                
                if features:
                    # 构建结果（包含6个核心HRV特征和质量标记）
                    result = {
                        ...
                        'peak_count': len(segment_peak_times),
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='从CSV文件中提取HRV特征（带质量评估）')
    ...
    # 创建特征提取器
    extractor = HRVFeatureExtractorWithQuality(segment_duration_ms=duration)
    ...
    print(f"   质量评分: {result['quality_score']:.4f}")
    ...
    if output_csv:
        ...
        # 按指定顺序排列列（包含6个核心特征和质量标记）
        cols = ['RMSSD', 'pNN58', 'SDNN', 'SD1', 'SD2', 'SD1_SD2', 'quality_status']
        df = df[cols]
        
        # 保存到文件（不包含表头和索引）
        df.to_csv(output_csv, index=False, header=False, encoding='utf-8-sig')
        print(f"💾 结果已保存到: {output_csv}")
        print(f"📄 输出格式: 6个特征值 + 质量标记")
